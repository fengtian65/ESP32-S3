...
│   ├── 07-timer/             # 定时器使用
│   ├── 08-tem_DHT11_CozeAI/  # 温湿度采集+扣子AI智能体实战
...
1. **克隆仓库**
	...
	cd ESP32-S3
	```
2. **编译与烧录** 确保已在 VS Code 中安装并配置好**ESP-IDF 插件** ，然后按以下可视化步骤操作（无需命令行）：
	1. **打开项目** ：在 VS Code 中打开任意例程项目文件夹（如`examples/01-LED/` ），等待 ESP-IDF 插件自动加载。
...
1.  打开`examples/08-tem_DHT11_CozeAI/main/main.c`
...
## 📝 更新日志
- 2026-02-24：新增「DHT11温湿度采集+扣子Coze智能体」进阶实战例程，修复基础例程若干bug
